...
const faqs = [
  ...
  ["Where will the hackathon take place?", "CODE FLUX will take place at Lovely Professional University, Punjab."],
] as const;
...
export function Footer() { return <footer className="border-t border-border bg-paper"><div className="mx-auto grid max-w-7xl gap-10 px-5 py-14 md:grid-cols-3 lg:px-8"><div><Wordmark/><p className="mt-4 text-xs font-bold uppercase tracking-[0.18em] text-muted-foreground">Build • Break • Innovate</p><p className="mt-4 flex items-center gap-2 text-sm text-muted-foreground"><MapPin className="size-4"/>Lovely Professional University, Punjab</p></div><div><h3 className="font-display text-sm font-bold uppercase text-ink">Quick links</h3><div className="mt-4 grid grid-cols-2 gap-3 text-sm text-muted-foreground">{[["Home","#home"],["About","#about"],["Tracks","#tracks"],["Timeline","#timeline"],["FAQs","#faqs"]].map(([l,h])=><a key={h} href={h} className="hover:text-ink">{l}</a>)}</div></div><div><h3 className="font-display text-sm font-bold uppercase text-ink">Follow us</h3><div className="mt-4 flex gap-3"><a href="https://www.instagram.com/codingblocks_lpu/" target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="grid size-10 place-items-center rounded-full border border-border text-ink transition hover:-translate-y-0.5 hover:border-ink"><Instagram className="size-4"/></a></div></div></div><div className="border-t border-border px-5 py-5 text-center text-xs text-muted-foreground">© 2026 CODE FLUX. All rights reserved.</div></footer> }